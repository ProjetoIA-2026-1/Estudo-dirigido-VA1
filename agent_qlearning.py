...
# Código do Gemini, comentei o teu antigo pra não perder pois não vou conseguir revisar
...
class AgenteQLearning:
    ...
    def __init__(
        ...
        # Hiperparâmetros otimizados: O Gamma alto (0.99) faz o agente priorizar o resgate futuro
        config_defaults = {
            "FACIL": {"alpha": 0.20, "gamma": 0.99, "decay": 0.992, "episodios": 600},
            "MEDIO": {"alpha": 0.15, "gamma": 0.99, "decay": 0.996, "episodios": 1500},
            "DIFICIL": {"alpha": 0.10, "gamma": 0.99, "decay": 0.998, "episodios": 3000}
        }
    ...
    def get_estado(self, visao_local):
        """
        NOVO ESTADO RELATIVO: Generalização Absoluta
        Ao invés de coordenadas X e Y exatas, o agente observa apenas a "cruz" ao seu redor
        e o vetor de direção para a linha de chegada.
        """
        cima = visao_local[1][2]
        baixo = visao_local[3][2]
        esquerda = visao_local[2][1]
        direita = visao_local[2][3]

        # Retorna -1, 0 ou 1 indicando para onde ele deve ir
        dir_x = np.sign(self.env.objetivo_x - self.env.posicao_x)
        dir_y = np.sign((self.env.comprimento - 1) - self.env.posicao_y)

        return (cima, baixo, esquerda, direita, dir_x, dir_y)

    def get_q_valores(self, estado):
        """
        Retorna os valores Q [Q(a0), Q(a1), Q(a2), Q(a3)] para o estado.
        Inicializa com 0.0 para que o ambiente nativo molde as decisões.
        """
        if estado not in self.q_tabela:
            self.q_tabela[estado] = [0.0, 0.0, 0.0, 0.0]
        return self.q_tabela[estado]
    ...
    def treinar_um_episodio(self):
        """
        Executa um episódio de treinamento recebendo a visão limpa do Environment.
        """
        visao_local = self.env.reset()
        estado_atual = self.get_estado(visao_local)

        pontuacao_total = 0
        chegou_objetivo = False
        passos = 0

        while passos < self.env.limite_passos:
            passos += 1

            acao = self.escolher_acao(estado_atual, explorando=True)
            visao_nova, recompensa_nativa, done, _ = self.env.step(acao)

            pontuacao_total += recompensa_nativa
            proximo_estado = self.get_estado(visao_nova)
            q_atual = self.get_q_valores(estado_atual)[acao]

            if done:
                alvo = recompensa_nativa
                if recompensa_nativa == 100:
                    chegou_objetivo = True
            else:
                max_q_proximo = max(self.get_q_valores(proximo_estado))
                alvo = recompensa_nativa + self.gamma * max_q_proximo

            self.q_tabela[estado_atual][acao] += self.alpha * (alvo - q_atual)
            estado_atual = proximo_estado
            visao_local = visao_nova
    ...
    def obter_melhor_acao(self, estado=None, visao_local=None):
        if estado is None and visao_local is not None:
            estado = self.get_estado(visao_local)
        return self.escolher_acao(estado, explorando=False)

    def planejar_rota(self):
        """
        Modo de Ação (Inferência Visual).
        Gera a rota final aplicando a Máscara de Sobrevivência para evitar bloqueios.
        """
        visao_local = self.env.reset()
        caminho_acoes = []
        done = False
        ...
        while not done and passos < self.env.limite_passos:
            ...
            # Puxa o instinto treinado da Tabela Q
            valores_q = list(self.get_q_valores(estado_atual))

            # Máscara de Ouro: Impede o agente de entrar voluntariamente em minas ou paredes na hora do Replay
            if visao_local[1][2] in [self.env.MINA, -1]: valores_q[0] = -99999
            if visao_local[2][1] in [self.env.MINA, -1]: valores_q[1] = -99999
            if visao_local[2][3] in [self.env.MINA, -1]: valores_q[2] = -99999
            if visao_local[3][2] in [self.env.MINA, -1]: valores_q[3] = -99999

            max_q = max(valores_q)
            melhores_acoes = [i for i, q in enumerate(valores_q) if q == max_q]
            acao = random.choice(melhores_acoes)

            # Trava Anti-Loop: Se tentar andar em círculos, pega a segunda melhor opção segura
            if (self.env.posicao_x, self.env.posicao_y, acao) in visitados:
                acoes_seguras = [a for a, q in enumerate(valores_q) if q > -90000]
                if acoes_seguras:
                    acao = random.choice(acoes_seguras)
                else:
                    break

            visitados.add((self.env.posicao_x, self.env.posicao_y, acao))
            caminho_acoes.append(acao)
