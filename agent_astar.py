...
class AgenteAStar:
    ...
    def planejar_rota(self, mapa_customizado=None):
        # NOVIDADE: Permite ao gerador passar um mapa "fantasma" para o A* testar
        mapa_alvo = mapa_customizado if mapa_customizado is not None else self.env.mapa_original
        ...
        while fila:
            ...
            for acao, nx, ny in acoes:
                if 0 <= nx < self.env.largura and 0 <= ny < self.env.comprimento:
                    ...
                    # LÊ DO MAPA ALVO (Oficial ou Fantasma)
